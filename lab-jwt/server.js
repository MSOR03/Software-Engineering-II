...
const server = http.createServer(async (req, res) => {
  ...
  if (method === "POST" && url === "/auth/register") {
    ...
    if (existe) return send(res, 409, { error: "El email ya esta registrado" });
  ...
  if (method === "POST" && url === "/auth/login") {
    ...
    if (!user) return send(res, 401, { error: "Credenciales invalidas" });
  ...
  if (method === "POST" && url === "/tasks") {
    ...
    if (!title) return send(res, 400, { error: "El titulo es requerido" });
